...
//发送邮件
function mailSend($mail_rec,$file){
	...
	$mail->Username = "imhzs@sina.com";// 发送方的126邮箱用户名
	$mail->Password = "5355408";// 发送方的邮箱密码，注意用126邮箱这里填写的是“客户端授权密码”而不是邮箱的登录密码！
	$mail->SMTPSecure = "ssl";// 使用ssl协议方式
	$mail->Port = 465;// 163邮箱的ssl协议方式端口号是465/994
	$mail->setFrom("imhzs@sina.com","数据库自动备份");// 设置发件人信息，如邮件格式说明中的发件人，这里会显示为Mailer(xxxx@126.com），Mailer是当做名字显示
	$mail->addAddress( $mail_rec ,'datebase_buck');// 设置收件人信息，如邮件格式说明中的收件人，这里会显示为Liang(yyyy@126.com)
	$mail->addReplyTo("imhzs@sina.com","Reply");// 设置回复人信息，指的是收件人收到邮件后，如果要回复，回复邮件将发送到的邮箱地址
...
if($cfg_type == 1){
		//邮箱备份
		mailSend($cfg_email,$to_file_name);
		unlink($to_file_name);
}else if($cfg_type == 2){
		//阿里云oss备份
